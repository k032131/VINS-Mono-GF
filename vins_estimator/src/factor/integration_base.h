...
class IntegrationBase
{
  public:
    ...
    {
        dt_buf.push_back(dt);//这个dt_buf和processIMU函数中push_back函数下面使用的dt_buf不是同一个变量
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
		...
		//里面求解了预积分、协方差、雅可比矩阵的更新,此处积分的是两帧之间的预计分，以前一帧图像坐标系为参考坐标系
    ...
    double sum_dt;
    Eigen::Vector3d delta_p;//两帧间IMU积分，相对于前一帧坐标系位置变化
    Eigen::Quaterniond delta_q;//两帧间IMU积分，相对于前一帧坐标系方向变化
    Eigen::Vector3d delta_v;//两帧间IMU积分，相对于前一帧坐标系速度变化
