/**
  * @brief 该文件主要包括论文VII A与B中的部分，实现描述子建立与回环检测 
  */
#include "keyframe.h"
...
// create keyframe online真正构造KeyFrame的是边缘化的帧的数据,从vins_estimator文件夹的visualization.cpp文件中的pubKeyframe函数中可以看出来
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    #if 1
	    	if (DEBUG_IMAGE)//调试输出使用的代码
	        {
	        	int gap = 10;
	        	cv::Mat gap_image(ROW, gap, CV_8UC1, cv::Scalar(255, 255, 255));
	            cv::Mat gray_img, loop_match_img;
	            cv::Mat old_img = old_kf->image;
				//将两张匹配的图像放在一张图像中显示，中间隔了一个宽为10像素的白线
	            cv::hconcat(image, gap_image, gap_image);
	            cv::hconcat(gap_image, old_img, gray_img);
	            cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);
				//分别在当前帧和之前的关键帧上将匹配上的点标出，并用线连接起来
	            for(int i = 0; i< (int)matched_2d_cur.size(); i++)
	            {
	                cv::Point2f cur_pt = matched_2d_cur[i];
	                cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 255, 0));
...
//关键帧在世界坐标系中的位姿,在pose_graph.cpp optimize4DoF函数中add edge代码中可以验证
