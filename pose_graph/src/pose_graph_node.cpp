...
{
    if (VISUALIZE_IMU_FORWARD)// default is 0
    {
...
void process()
{
    ...
    while (true)
    {
        sensor_msgs::ImageConstPtr image_msg = NULL;//主要包括图像的时间戳，长宽等图像信息
        sensor_msgs::PointCloudConstPtr point_msg = NULL;//图像中的路标点
        nav_msgs::Odometry::ConstPtr pose_msg = NULL;//指的是图像帧的位姿
        ...
        if (pose_msg != NULL)
        {
            ...
            //printf(" image time %f \n", image_msg->header.stamp.toSec());
            // skip fisrt few
            //剔除前SKIP_FIRST_CNT帧，然后每隔SKIP_CNT帧进行一次
...
int main(int argc, char **argv)
{
    ...
    if (LOOP_CLOSURE)
    {
        ...
        VISUALIZE_IMU_FORWARD = fsSettings["visualize_imu_forward"];// default is 0
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 2000, imu_forward_callback);//默认不执行
