...
PoseGraph::PoseGraph()
{
	...
	//Relocalization部分，检测到回环后会对窗口位姿进行微调，从而对齐各个帧，这个可以从VIII的第一句话可以知道。变量中的drift应该指的这个微调的意思
...
//添加关键帧，并且根据当前关键帧与最早满足score条件的关键帧的位姿（该值在之前已经得到）来计算偏移，并且将这个偏移用在所有的keyframelist中
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop/*0:加载;1:新建*/)
{
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    ...
    cur_kf->index = global_index;//窗口外的序号
    global_index++;
	int loop_index = -1;
    if (flag_detect_loop)
    {
...
/**@brief判断与当前帧匹配的回环帧
  *
  *返回的是满足score得分>=0.015的所有关键帧中id最小的帧
  */
...
//pose graph的优化只对第一次检测到回环之后的关键帧的位姿进行了优化，之前的帧没有优化
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            int max_length = cur_index + 1;//cur_index是当前关键帧序列中标号最大的帧
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                //add edge
                //添加sequential edge，最多只与当前关键帧之前的4个关键帧建立边
                for (int j = 1; j < 5; j++)
                {
